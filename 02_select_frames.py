...
def process_shot_gpu(shot_id, seg_start, seg_end,
    ...
    # ── 强制覆盖：跳过首尾过渡区(0.3s)，镜头内部分段均匀覆盖 ──
    extra = []
    if len(seg_mafd) > 0 and dur_s > 2:
        skip_frames = max(1, int(fps * 0.3))
        body_s = skip_frames                    # 镜头内部起点（相对）
        body_e = max(body_s + 1, n_frames - skip_frames)  # 镜头内部终点（相对）

        if body_e > body_s:
            n_segs = min(8, max(2, int(dur_s / 8)))  # 每 8s 一段
            seg_len = (body_e - body_s) // n_segs
            for si in range(n_segs):
                lo = body_s + si * seg_len
                hi = body_s + (si + 1) * seg_len if si < n_segs - 1 else body_e
                lo_abs = seg_start + lo
                hi_abs = seg_start + hi
                if any(lo_abs <= kf <= hi_abs for kf in key_frames):
                    continue
                if hi > lo and lo < len(seg_mafd):
                    z_end = min(hi, len(seg_mafd))
                    zone = seg_mafd[lo:z_end]
                    if len(zone) > 0:
                        best = int(np.argmin(zone)) + seg_start + lo
                        if best not in key_frames and best not in [e[0] for e in extra]:
                            extra.append((best, 0.0))

    if extra:
        key_frame_scores.extend(extra)
        key_frame_scores.sort(key=lambda x: x[0])
        key_frames = [f for f, _ in key_frame_scores]
